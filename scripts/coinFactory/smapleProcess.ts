...
async function main() {
  ...
  const P12FactoryProxy = await P12CoinFactoryUpgradeable.attach('0x395FAbef71433280f85f79ad43f99E3cC040af5C');
  ...
  // get pair
  const P12GameCoin = await ethers.getContractFactory();
  const gameCoin = await P12GameCoin.attach(String(gameCoinAddress));
  ...
  console.log('P12GameCoin Pair Reserves: ', await pool.getReserves());
  ...
  console.log('p12CoinFactory delay K: ', await P12FactoryProxy.delayK());
  console.log('p12CoinFactory delay B: ', await P12FactoryProxy.delayB());
