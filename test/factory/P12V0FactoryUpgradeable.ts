...
describe("P12Factory", function () {
  ...
  this.beforeAll(async function () {
    ...
    UniswapV2Factory = await UNISWAPV2FACTORY.connect(admin).deploy(
      admin.address
    );

    const weth = await WETH.deploy();

    router = await UNISWAPV2ROUTER.connect(admin).deploy(
  ...
  it("Should show p12 token deploy successfully!", async function () {
    ...
    p12 = await ERC20.connect(admin).deploy(
      "ProjectTwelve",
      "P12",
      1000n * 10n ** 18n
    );
  ...
  it("Should show P12Factory contract deploy successful!", async function () {
    const P12FACTORY = await ethers.getContractFactory(
      "P12V0FactoryUpgradeableV2"
    );
    p12Factory = await upgrades.deployProxy(
      P12FACTORY,
      [p12.address, UniswapV2Factory.address, router.address, 3600],
      { kind: "uups" }
    );
  ...
  it("show deploy p12mine successfully", async function () {
    ...
    const delayK = 60;
    const delayB = 60;
    const P12MineUpgradeable = await ethers.getContractFactory(
      "P12MineUpgradeable"
    );
  ...
  it("Give developer p12", async function () {
    await p12
      .connect(admin)
      .transfer(developer.address, BigInt(3) * 10n ** 18n);
  ...
  it("Should show gameCoin create successfully!", async function () {
    ...
    (await createInfo.wait()).events!.forEach(
      (x: { event: string; args: any }) => {
  ...
  it("Should show duplicate mint fail!", async function () {
    await new Promise((resolve) => setTimeout(() => resolve(true), 5000));
    await expect(
      p12Factory.executeMint(gameCoinAddress, mintId)
    ).to.be.revertedWith("this mint has been executed");
  });
