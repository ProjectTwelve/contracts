...
describe('VotingEscrow', function () {
  ...
  it('withdraw p12Token Emergency successfully', async function () {
    await expect(core.votingEscrow.connect(test).expire()).to.be.revertedWith('SafeOwnable: caller not the owner');
    await expect(core.votingEscrow.connect(test).withdraw()).to.be.revertedWith('VotingEscrow: The lock did not expire');
